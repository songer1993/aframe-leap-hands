AFRAME.registerComponent('fuse-cursor', {
  ...
  init: function () {
    ...
    var fuse = this.fuse = this.data.fuse; // true if cursor fuse is enabled.
    var fuseTimeout = this.data.fuseTimeout; // animation lenght should be based on this value
    ...
    if (data.design == 'dot') {
      ...
      clickAnimation.setAttribute('begin', 'fuseend');
    ...
    if (fuse) {
      el.addEventListener('mouseenter', function (e) {
        function hasComponents(el, components) {
          for (let i = 0; i < components.length; i++) {
            if (!el.components.hasOwnProperty(components[i])) {
              return false;
            }
          }
          return true;
        }
        let fuseFilters = data.fuseFilters;
        let intersectedEl = e.detail.intersectedEl;
        while (!hasComponents(intersectedEl, fuseFilters)) {
          intersectedEl = intersectedEl.parentEl;
          if (intersectedEl.classList.contains('a-body')) return;
        }

        fuseLoader.emit('start-fusing');
        el.emit('fuse');
        that.fuseTimer = setTimeout(function fuse() {
          el.emit('fuseend');
          intersectedEl.emit('fuseend');
        }, fuseTimeout);
      });
    }
...
AFRAME.registerPrimitive('a-fuse-cursor', {
  defaultComponents: {
    'cursor': {},
    'fuse-cursor': {}
  },
  mappings: {
    'fuse': 'fuse-cursor.fuse',
    'fuse-timeout': 'fuse-cursor.fuseTimeout',
    'fuse-filters': 'fuse-cursor.fuseFilters',
    'color': 'fuse-cursor.color',
    'hover-color': 'fuse-cursor.hoverColor',
    'active-color': 'fuse-cursor.activeColor',
    'distance': 'fuse-cursor.distance',
    'design': 'fuse-cursor.design'
  }
});